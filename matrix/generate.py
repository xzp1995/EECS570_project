...
for i in range(0,N):
    ...
    matrix[2 * i + 1] = np.random.randint(LOW, HIGH, size = (W,H), dtype=int)
    b = np.zeros((W + H, H)).astype(int)
    for j in range(0,W):
        for k in range(0, H):
            b[j + k, j] = matrix[2 * i + 1, k, j]
    np.savetxt("ob_{}.txt".format(i), matrix[2 * i + 1].reshape(W * H, 1), fmt = '%d')
    np.savetxt("b_{}.txt".format(i), b.reshape((W + H) * H, 1), fmt = '%d')
